...
struct ScoredNumberModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            .shadow(color: Color("ColorTransparentbBlack"), radius: 0, x: 0, y: 3)
            .layoutPriority(1)
    }
}
...
struct BetNumberModifier: ViewModifier {
    func body(content: Content) -> some View {
        content
            ...
            .padding(.vertical,5)
            .frame(width: 90)
            .shadow(color: Color("ColorTrasnparentBlack"), radius: 0, x: 0, y: 3)
    }
}
